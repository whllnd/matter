...
for i in range(N):
    if i == 0:
        particles.append(Particle(massHi, np.array([0.,0.])))
    else:
        particles.append(Particle(massLo, initPos(xlim, ylim)))
...
while True:
    ...
    plt.scatter([p.pos[0] for p in particles], [p.pos[1] for p in particles], c=np.arange(N))
    plt.xlim(-5, 5)
    plt.ylim(-5, 5)
    plt.show()
